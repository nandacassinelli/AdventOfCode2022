...
const moveCrates = (cratesStacks, origin, destination, quantity) => {
  cratesStacks[destination]
    .push(...cratesStacks[origin].splice(cratesStacks[origin].length - quantity, quantity))
...
commands
  ...
  .map(command => {
    stacks = moveCrates(stacks, +command[3] - 1, +command[5] - 1, +command[1])
  })
